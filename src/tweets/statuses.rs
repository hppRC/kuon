...
impl TwitterAPI {
    pub async fn home_timeline(&self, params: &HashMap<&str, &str>) -> Result<Vec<Tweet>, Error> {
        let endpoint = "https://api.twitter.com/1.1/statuses/home_timeline.json";
        self.raw_get(endpoint, &params).await
    }

    pub async fn user_timeline(&self, params: &HashMap<&str, &str>) -> Result<Vec<Tweet>, Error> {
